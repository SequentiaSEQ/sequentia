...
private:
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state, CAsset& fee_asset) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        ...
        CAmount min_relay_tx_fee = ::minRelayTxFee.GetFee(package_size, fee_asset);
        if ( package_fee < min_relay_tx_fee) {
            return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "min relay fee not met", strprintf("%d < %d", package_fee, min_relay_tx_fee));
        }
        return true;
    }
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!bypass_limits && !CheckFeeRate(package_size, ws.m_modified_fees, state, feeAsset)) return false;
